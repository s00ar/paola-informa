...
const moveDirectories = async (userInput) => {
  try {
    if (userInput === 'y') {
      ...
      console.log(`[info] /${exampleDir} directory created.`);
    }
    ...
    console.log('\n[info] New /app directory created.');

    const indexPath = path.join(newAppDirPath, 'index.jsx');
    await fs.promises.writeFile(indexPath, indexContent);
    console.log('[info] app/index.jsx created.');

    const layoutPath = path.join(newAppDirPath, '_layout.jsx');
    await fs.promises.writeFile(layoutPath, layoutContent);
    console.log('[info] app/_layout.jsx created.');

    console.log('\n[done] Project reset complete. Next steps:');
    console.log(
